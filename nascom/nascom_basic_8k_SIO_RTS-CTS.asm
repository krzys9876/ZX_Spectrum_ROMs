...
; c/~d=addr0
; a/~b=addr1
; TxC, RxC=CLK/2 / 64 = 28800, /32 = 57600, /16 = 115200
...
TEMPSTACK       .EQU     20EDH ; Top of BASIC line input buffer so is "free ram" when BASIC resets
...
RST00           DI                       ;Disable interrupts
                JP       INIT1            ;Initialize Hardware and go
...
notWrap:        LD       (serInPtr),HL
                ...
                CP       SER_FULLSIZE
                JR       C,rts0
; rts wy³¹czony - powinien byæ w³¹czony po odebraniu wszystkich znaków z bufora
;		call rtsOFF
...
TXA:            PUSH     AF              ; Store character
		call tx_emp
		call ctsON ; oczekiwanie na pozwolenie na nadawanie
...
;SIO init
	...
	ld a,04h ; select WR4 
	out (sio_ac),a
	ld a,01001100b ; no parity, 1 stop bit, 16x clock (zegar z CLK/2, 115200)
	out (sio_ac),a
	ld a,03h ; select WR3
	out (sio_ac),a
	ld a,11100001b ; 8 bits, auto enable, Rx enable
	out (sio_ac),a
	ld a,05h ; select WR5
	out (sio_ac),a
	ld a,11101010b ; enable DTR, RTS, enable transfer,8 bits
	out (sio_ac),a
;INIT cont.
...
;TST_MSG1:       .DEFB	 "I",CR,LF,0	
;TST_MSG2:       .DEFB	 "C",CR,LF,0	
;TST_MSG3:       .DEFB	 "W",CR,LF,0	
;TST_MSG4:       .DEFB	 "04",CR,LF,0	
...
INITABE1  .EQU INITBE-INITAB ; KR
INITABE2  .EQU INITABE1+3 ; KR
...
;WARM:   JP      STARTB          ; Jump for warm start
...
INIT:   LD      DE,INITAB       ; Initialise workspace
        LD      B,INITABE2; Bytes to copy ; INITBE-INITAB+3
...
COPY:   LD      A,(DE)          ; Get source
        ...
        LD      (BUFFER2),A ; Mark end of buffer ; BUFFER+72+1
...
MLOOP:  INC     HL              ; Next byte
        ...
        LD      (HL),B          ; Restore old contents !!!! KR
	;ld (HL),0		;!!!!!!!!!!! KR zerowanie pamiêci
...
BFREE:  .DEFB   " Bytes free"
        .DEFB   CR,LF,0,0

SIGNON: .DEFB   "Z80 BASIC "
        .DEFB   "Ver 4.7b"
        .DEFB   CR,LF
        .DEFB   "Copyright "
        .DEFB   40,"C",41
...
ZNEW1 .EQU ZNEW+1 ; KR
ZNEWEND .EQU ZNEW1-ZEND ; KR

        CP      ZNEWEND     ; END to NEW ?
...
GTLNLP: CALL    GETCHR          ; Get next character
        ...
        PUSH    HL              ; Save code string address
        PUSH    AF              ; Save digit
        LD      HL,1998H ; KR 65529/10     ; Largest number 65529
...
INPUT:  CALL    IDTEST          ; Test for illegal direct
        ...
        CP      34 ;'"'             ; Is there a prompt string?
...
ZOR1   .EQU     ZOR+1 ; KR
ZORPLUS .EQU    ZOR1-ZPLUS ; KR

        CP      ZORPLUS     ; Is it + - * / ^ AND OR ?
        RET     NC              ; No - Exit
...
ZLEFTSGN .EQU  ZLEFT-ZSGN ;KR
ZLEFTSGN2 .EQU 2*ZLEFTSGN ;KR
ZLEFTSGN21 .EQU ZLEFTSGN2-1 ;KR

        CP      ZLEFTSGN21; Adj' LEFT$,RIGHT$ or MID$ ?
...
QTSTR:  LD      B,34 ;'"'           ; Terminating quote
        LD      D,B             ; Quote to D
...
; KR -------------------------------------------------------------------
	...
	; oczekiwanie na zakoñczene wysy³ania znaku		
tx_emp:	ld a,01h
	out (sio_ac),a
	in a,(sio_ac)
	;ld a,1 ;!!!!!
	bit 0,a ; bit 0 w RR1=1--> wszystkie znaki zosta³y wys³ane
	jr z,tx_emp
	ret

	; oczekiwanie na znak w buforze (wczeœniej trzeba w³¹czyæ RTS)
rx_av:	ld a,00h
	out (sio_ac),a
	in a,(sio_ac)
	;ld a,1 ;!!!!!	
	bit 0,a ; bit 0 w RR0=1-> jest znak do pobania
	;jr z,rx_av ; w wersji z INT nie mo¿na blokowaæ programu
	ret

	; oczekiwanie na CTS - pozwolenie na nadawanie
ctsON:	ld a,00h
	out (sio_ac),a
	in a,(sio_ac)
	;ld a,20h ;!!!!!	
	bit 5,a ; bit 5 w RR0 odpowiada sygna³owi CTS, czekamy na 1
	jr z,ctsON
	ret		

	; warunkowe w³¹czenie RTS - tylko wtedy, gdy nie ma znaków w buforze
rtsON:  ld a,00h
	out (sio_ac),a
	in a,(sio_ac)
	;ld a,1 ;!!!!!	
	bit 0,a ; bit 0 w RR0=1-> jest znak do pobrania, wyjœcie bez ustawienia RTS
	jr nz,rtsONret 
	ld a,05h ; select WR5
	out (sio_ac),a
	ld a,11101010b ; enable DTR, RTS on, enable transfer,8 bits
	out (sio_ac),a
rtsONret:  
	ret
	
	; wy³¹czenie RTS - nadawca nie mo¿e nic wys³aæ	
rtsOFF: ld a,05h ; select WR5
	out (sio_ac),a
	ld a,11101000b ; enable DTR, RTS off, enable transfer,8 bits
	out (sio_ac),a
	ret
...
mchloop:	
	...
	CPL
	LD (HL),A
	INC A ; do symulacji - zmiana A
	LD A,(HL)
	CP B ; do symulacji - wyzerowanie flagi Z
	CPL
	CP B ; sprawdzenie z pierwotn¹ zawartoœci¹
	LD (HL),B
	JR Z,noerr
err:	INC C	
	LD D,04Eh ; "N"
noerr:	
	...
	LD A,0
	CP H
	JR Z,zeroH ; koniec po przekroczeniu FFFF
	JP mchloop
zeroH:	CP L
	JR Z,endt
	JP mchloop
